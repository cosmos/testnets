---
title: Neutron v1.1.0 Upgrade
order: 2
---
<!-- markdown-link-check-disable -->

# Neutron v1.1.0 Upgrade, Instructions

* Chain upgrade point: TODO: DETERMINE AND ADD HEIGHT AND DATE
* Go version: v1.20
* Release: TODO: ADD A LINK TO THE RELEASE

This document describes the steps for validators and full node operators, to upgrade successfully to the Neutron v1.1.0 release. For more details on the release, please see the [release notes](TODO:ADD A LINK TO THE RELEASE).

## On-chain governance proposal attains consensus

[Proposal N](TODO: ADD A LINK TO THE PROPOSAL) is the reference on-chain governance proposal for this upgrade. Neither core developers nor core funding entities control the governance, and this governance proposal has passed in a _fully decentralized_ way.

## Upgrade date

The upgrade will take place at a block height of `TODO: ADD THE DETERMINED BLOCK HEIGHT`. The date/time of the upgrade is subject to change as blocks are not generated at a constant interval. You can stay up-to-date using this [live countdown](https://www.mintscan.io/neutron/blocks/TODO: ADD THE DETERMINED BLOCK HEIGHT) page.
...
Prior to the upgrade, validators are encouraged to take a full data snapshot. Snapshotting depends heavily on infrastructure, but generally this can be done by backing up the `.neutron` directory.
...
It is critically important for validator operators to back-up the `.neutron/data/priv_validator_state.json` file after stopping the neutrond process. This file is updated every block as your validator participates in consensus rounds. It is a critical file needed to prevent double-signing, in case the upgrade fails and the previous chain needs to be restarted.
...
The Neutron testnet network, `pion-1`, will run [Neutron v1.1.0](TODO: ADD A LINK TO THE RELEASE). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
Make sure Neutron v1.1.0 is installed by either downloading a [compatible binary](TODO: ADD A LINK TO THE RELEASE), or building from source. Building from source requires **Golang 1.20.x**.

Run Neutron v1.0.4 till upgrade height, the node will panic:

TODO: REPLACE "h" with actual values and make sure "v1.1.0" is the precise version name
```shell
ERR UPGRADE "v1.1.0" NEEDED at height: h: upgrade to v1.1.0 and applying upgrade "v1.1.0" at height:h
```

Stop the node, and switch the binary to Neutron v1.1.0 and re-start by `neutrond start`.
...
build Neutron v1.1.0, and move neutrond v1.1.0 to `$NEUTRON_HOME/cosmovisor/upgrades/v1.1.0/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v1.1.0/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v1.1.0/bin
```
...
Then the Cosmovisor will create `$NEUTRON_HOME/cosmovisor/upgrades/v1.1.0/bin` and download the Neuton v1.1.0 binary to this folder according to links in the `--info` field of the upgrade proposal.
...
The upgrade may take a few minutes to several hours to complete because pion-1 participants operate globally with differing operating hours and it may take some time for operators to upgrade their binaries and connect to the network.
...
TODO: REPLACE "h" with actual value
> neutrond start --unsafe-skip-upgrade h
