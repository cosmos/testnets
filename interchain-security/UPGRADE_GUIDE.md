...
2. Wait for the node to stop at the upgrade height.
   * The log will display something like this:
     ```
     ERR UPGRADE "v23" NEEDED at height: <UPGRADE_HEIGHT>: upgrade to v23 and applying upgrade "v23" at height:<UPGRADE_HEIGHT>
     ```
...
2. Create a folder for the new binary in the relevant Cosmovisor directory.
   * If the upgrade name is `v23`, you would place the binary under `<node home>/cosmovisor/upgrades/v23/bin/gaiad`:
     ```
     .
