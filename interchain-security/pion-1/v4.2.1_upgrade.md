...
* Chain upgrade point: `22th of August 15:30 UTC` at height `18256000`
* Go version: `v1.22`
...
The Neutron testnet network, `pion-1`, is currently running [Neutron v4.0.1-testnet](https://github.com/neutron-org/neutron/releases/tag/v4.0.1-testnet). We anticipate that operators who are running on v4.0.1-testnet, will be able to upgrade successfully. Validators are expected to ensure that their systems are up-to-date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.22`.

### Target runtime

The Neutron testnet network, `pion-1`, will run [Neutron v4.2.1-testnet](https://github.com/neutron-org/neutron/releases/tag/v4.2.1-testnet). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
Make sure Neutron v4.2.1-testnet is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v4.2.1-testnet), or building from source. Building from source requires **Golang 1.22.x**.

Run Neutron v4.0.1-testnet till upgrade height, the node will panic:

```shell
ERR UPGRADE "v4.2.1-testnet" NEEDED at height: 18256000: upgrade to v4.2.1-testnet and applying upgrade "v4.2.1-testnet" at height:18256000
```

Stop the node, and switch the binary to Neutron v4.2.1-testnet and re-start by `neutrond start`.
...
create a Cosmovisor folder inside `$NEUTRON_HOME` and move Neutron v4.0.1-testnet into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
build Neutron v4.2.1-testnet, and move neutrond v4.2.1-testnet to `$NEUTRON_HOME/cosmovisor/upgrades/v4.2.1-testnet/bin`

```shell
mkdir -p  $NEUTRON_HOME/cosmovisor/upgrades/v4.2.1-testnet/bin
cp $(which neutrond) $NEUTRON_HOME/cosmovisor/upgrades/v4.2.1-testnet/bin
```
...
create a cosmovisor folder inside neutron home and move neutrond v4.0.1-testnet into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
```shell
ERR UPGRADE "v4.2.1-testnet" NEEDED at height: 18256000: upgrade to v4.2.1-testnet and applying upgrade "v4.2.1-testnet" at height:
```

Then the Cosmovisor will create `$NEUTRON_HOME/cosmovisor/upgrades/v4.2.1-testnet/bin` and download the Neuton v4.2.1-testnet binary to this folder according to links in the `--info` field of the upgrade proposal.
...
- Users should run their node on v4.0.1-testnet if they use the cosmovisor v1.5.0 with auto-download enabled for upgrade process.
...
Steps to skip this upgrade proposal are simply to resume the pion-1 network with the (downgraded) v4.0.1-testnet binary using the following command:

> neutrond start --unsafe-skip-upgrade 18256000
