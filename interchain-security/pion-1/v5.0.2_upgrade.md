...
* **Upgrade your Slinky SideCar to version v1.0.13**
* Chain upgrade point: `11th of December 14:30 UTC` at height `22119000`
* Go version: `v1.22`
...
The Neutron testnet network, `pion-1`, is currently running [Neutron v5.0.0-rc0](https://github.com/neutron-org/neutron/releases/tag/v5.0.0-rc0). We anticipate that operators who are running on v5.0.0-rc0, will be able to upgrade successfully. Validators are expected to ensure that their systems are up-to-date and capable of performing the upgrade. This includes running the correct binary, or if building from source, building with go `1.22`.

### Target runtime

The Neutron testnet network, `pion-1`, will run [Neutron v5.0.2](https://github.com/neutron-org/neutron/releases/tag/v5.0.2). Operators _**MUST**_ use this version post-upgrade to remain connected to the network.
...
Make sure Neutron v5.0.2 is installed by either downloading a [compatible binary](https://github.com/neutron-org/neutron/releases/tag/v5.0.2), or building from source. Building from source requires **Golang 1.22.x**.

Run Neutron v5.0.0-rc0 till upgrade height, the node will panic:

```shell
ERR UPGRADE "v5.0.2" NEEDED at height: 22119000: upgrade to v5.0.2 and applying upgrade "v5.0.2" at height:22119000
```
...
create a Cosmovisor folder inside `$NEUTRON_HOME` and move Neutron v5.0.0-rc0 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
create a cosmovisor folder inside neutron home and move neutrond v5.0.0-rc0 into `$NEUTRON_HOME/cosmovisor/genesis/bin`
...
Then the Cosmovisor will create `$NEUTRON_HOME/cosmovisor/upgrades/v5.0.2/bin` and download the Neuton v5.0.2 binary to this folder according to links in the `--info` field of the upgrade proposal.
...
- Users should run their node on v5.0.0-rc0 if they use the cosmovisor v1.5.0 with auto-download enabled for upgrade process.
...
Steps to skip this upgrade proposal are simply to resume the pion-1 network with the (downgraded) v5.0.0-rc0 binary using the following command:
